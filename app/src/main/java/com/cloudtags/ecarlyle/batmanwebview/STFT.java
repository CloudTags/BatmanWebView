...
//import com.cloudtags.ecarlyle.myfirstapp.RealDoubleFFT;

/**
 * Created by ecarlyle on 12/19/16.
 */
...
public class STFT {
    ...
    public HashMap<Integer, Double> foundfreq =  new HashMap<Integer, Double>();

    public void findAssignedFrequencies(ArrayList assigned_freqs, int dblevel){
        getSpectrumAmpDB();
        // If spectrumAmpOutDB is all zeros then continue
        maxAmpDB = 20 * Math.log10(0.125/32768);
        double minAmpDB = 0;
        for(int cycle = 0; cycle < spectrumAmpOutDB.length; cycle++){
            if (spectrumAmpOutDB[cycle] > maxAmpDB){
                maxAmpDB  = spectrumAmpOutDB[cycle];
            }
            if (spectrumAmpOutDB[cycle] < minAmpDB){
                minAmpDB  = spectrumAmpOutDB[cycle];
            }
        }

        foundfreq = new HashMap<Integer, Double>();

        if(maxAmpDB==minAmpDB){
            // All of the results are the same
            return;
        }


        for(int counter = 0; counter < assigned_freqs.size(); counter++){
            int find_freq = (Integer) assigned_freqs.get(counter);
            int arraynumber = (int) Math.floor(find_freq*fftLen/sampleRate);
            //Log.d("arraynumber", String.valueOf(find_freq*fftLen/sampleRate));
            Log.d("Search Db:", String.valueOf(spectrumAmpOutDB[arraynumber]));
            if(spectrumAmpOutDB[arraynumber] > dblevel || spectrumAmpOutDB[arraynumber-1] > dblevel || spectrumAmpOutDB[arraynumber+1] > dblevel){
                Log.d("Found Freq:", String.valueOf(find_freq ));
                Log.d("Found DB:", String.valueOf(spectrumAmpOutDB[arraynumber] ));
                foundfreq.put(find_freq, spectrumAmpOutDB[arraynumber]);
